		...
		for _, block := range blocks {
			...
			for index, line := range lines {
				...
				if y >= selection.startY && y <= selection.endY {
					...
					selectedText := ansi.Cut(line, left, right)
					// Convert ANSI-styled text to markdown-friendly format
					markdownText := ansiToMarkdown(selectedText)
					middle := strings.TrimRight(markdownText, " ")
					suffix := ansi.Cut(line, left+ansi.StringWidth(selectedText), width)
...
func (m *messagesComponent) CopyLastMessage() (tea.Model, tea.Cmd) {
	...
	// Extract full markdown content from the last message
	markdownContent := extractMarkdownContent(m.app, lastMessage, m.showToolDetails, m.width)
	if markdownContent == "" {
		return m, nil
	}

	var cmds []tea.Cmd
	cmds = append(cmds, app.SetClipboard(markdownContent))
