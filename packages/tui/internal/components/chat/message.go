...
// extractMarkdownContent extracts the original markdown content from a message for clipboard
func extractMarkdownContent(
	...
	if _, ok := message.Info.(opencode.UserMessage); ok {
		for _, part := range message.Parts {
			switch p := part.(type) {
			case opencode.TextPart:
				if p.Synthetic || p.Text == "" {
					continue
				}
	...
	if _, ok := message.Info.(opencode.AssistantMessage); ok {
		for _, part := range message.Parts {
			switch p := part.(type) {
			case opencode.TextPart:
				...
				if strings.TrimSpace(p.Text) == "Generating..." {
					continue // Skip generation indicators
				}
				content.WriteString(p.Text)
				content.WriteString("\n\n")
			case opencode.ToolPart:
				...
				toolContent := extractToolMarkdown(app, p, width)
				if toolContent != "" {
	...
	result := strings.TrimSuffix(content.String(), "\n\n")
	return strings.TrimSuffix(result, "\n")
}

// extractToolMarkdown extracts markdown content from a tool part
func extractToolMarkdown(
	_ *app.App,
	toolCall opencode.ToolPart,
	_ int,
) string {
	if toolCall.State.Status == opencode.ToolPartStateStatusPending {
		return fmt.Sprintf("**%s** *(pending)*", renderToolAction(toolCall.Tool))
	}
	...
	toolInputMap := make(map[string]any)
	if toolCall.State.Input != nil {
		if m, ok := toolCall.State.Input.(map[string]any); ok {
			toolInputMap = m
		}
	}

	var content strings.Builder
	content.WriteString(fmt.Sprintf("**%s**", renderToolName(toolCall.Tool)))

	// Add tool arguments
	if len(toolInputMap) > 0 {
		args := renderArgs(&toolInputMap, "")
		if args != "" {
	...
	if toolCall.State.Metadata != nil {
		...
		switch toolCall.Tool {
		case "read":
			if preview, ok := metadata["preview"].(string); ok && toolInputMap["filePath"] != nil {
				filename := toolInputMap["filePath"].(string)
				content.WriteString(fmt.Sprintf("```%s\n%s\n```", util.Extension(filename), preview))
			}
		case "edit":
			if _, ok := toolInputMap["filePath"].(string); ok {
				if diffField, ok := metadata["diff"].(string); ok && diffField != "" {
					content.WriteString(fmt.Sprintf("```diff\n%s\n```", diffField))
				}
			}
		case "write":
			if filename, ok := toolInputMap["filePath"].(string); ok {
				if fileContent, ok := toolInputMap["content"].(string); ok {
					content.WriteString(fmt.Sprintf("```%s\n%s\n```", util.Extension(filename), fileContent))
				}
			}
		case "bash":
			if command, ok := toolInputMap["command"].(string); ok {
				content.WriteString(fmt.Sprintf("```console\n$ %s", command))
		...
		case "todowrite":
			if todos, ok := metadata["todos"].([]any); ok {
				for _, item := range todos {
					if todo, ok := item.(map[string]any); ok {
						if todoContent, ok := todo["content"].(string); ok && todoContent != "" {
							switch todo["status"] {
							case "completed":
	...
	if toolCall.State.Status == opencode.ToolPartStateStatusError && toolCall.State.Error != "" {
		content.WriteString(fmt.Sprintf("\n**Error:** %s", toolCall.State.Error))
	}

	return content.String()
}

// ansiToMarkdown converts ANSI-styled text back to markdown format
func ansiToMarkdown(text string) string {
	// Remove ANSI escape sequences but preserve semantic meaning
	text = ansi.Strip(text)

	// Basic markdown preservation - this is a simplified approach
	// In a full implementation, you'd want to map specific ANSI styles back to markdown
	lines := strings.Split(text, "\n")
	var result strings.Builder

	for i, line := range lines {
		// Preserve code blocks (detect by common patterns)
		if strings.Contains(line, "```") ||
			strings.HasPrefix(strings.TrimSpace(line), "$ ") ||
			strings.Contains(line, "[✓]") ||
			strings.Contains(line, "[ ]") {
			// This looks like code or structured content
