import { TextAttributes, useTerminalDimensions, useRenderer, useKeyboard } from "@opentui/solid"
...
let nativeLibraryError: Error | null = null

async function detectNativeLibrarySupport() {
  try {
    renderModule = await import("@opentui/solid")
    return true
  } catch (error) {
    nativeLibraryError = error instanceof Error ? error : new Error(String(error))
    return false
  }
}

function showNativeLibraryError(onExit: () => void | Promise<void>) {
  console.error("\n❌ Native Render Library Loading Failed")
  console.error("\n📋 Error Details:")
  if (nativeLibraryError) {
    console.error(`   ${nativeLibraryError.message}`)
  }
  
  console.error("\n🖥️  System Information:")
  console.error(`   Platform: ${process.platform}-${process.arch}`)
  console.error(`   Node.js: ${process.version}`)
  console.error(`   Terminal: ${process.env.TERM || 'unknown'}`)
  
  console.error("\n🔧 Possible Solutions:")
  console.error("   1. Try running in a different terminal")
  console.error("   2. Ensure your system supports terminal UI applications")
  console.error("   3. Check if all native dependencies are installed")
  console.error("   4. Try the web interface instead: https://opencode.ai")
  
  console.error("\n📚 For more help:")
  console.error("   Documentation: https://docs.opencode.ai")
  console.error("   Issues: https://github.com/sst/opencode/issues")
...
export function tui(input: {
  ...
  return new Promise<void>(async (resolve) => {
    const hasNativeSupport = await detectNativeLibrarySupport()
    if (!hasNativeSupport) {
      showNativeLibraryError(input.onExit || resolve)
      return
    }
